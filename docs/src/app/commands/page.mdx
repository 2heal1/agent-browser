...
`open`, `goto`, and `navigate` wait up to 25 seconds for the selected navigation lifecycle event by default. Pass `--timeout <ms>` to override that wait for one navigation. The effective timeout is also forwarded to the command transport with an additional response margin.
