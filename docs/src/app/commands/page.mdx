...
agent-browser open <url> [--timeout <ms>] # Launch + navigate (aliases: goto, navigate)
...
CPU throttling uses a slowdown factor of `1` or greater. Network values use decimal kilobits per second. Omitted network options keep their current values. Offline mode preserves configured latency and throughput limits, while `network-throttling reset` restores an online, zero-latency, unlimited connection.

These controls require Chromium CDP. They persist across navigation and apply to new tabs, popups, and cross-origin iframes. CPU throttling is not complete low-end device emulation and does not simulate memory, GPU, disk, core count, or thermal behavior.
...
agent-browser webmcp call <tool-name> --input '{}' --json
agent-browser webmcp call <tool-name> --input '{}' --frame-id <id> --timeout 5000 --json
```

These Chrome-only commands use the experimental WebMCP CDP domain to discover and call tools registered by the active page. WebMCP must be enabled before Chrome starts. Chrome 149 needs `WebMCPTesting` and `DevToolsWebMCPSupport`; Chrome 150 and newer use `WebMCP`. Enable all three names during the transition.

`list` returns tool schemas, annotations, frame IDs, and imperative/declarative source. `call` waits for the matching invocation response and cancels it on timeout. Returned tool output is untrusted page content and may contain prompt injection; annotations are hints, not enforcement. Use `--action-policy` or `--confirm-actions webmcp_call` when calls require approval.
...
agent-browser debug scripts           # List loaded compiled script instances
agent-browser debug source search <text> [--filter <url>]
agent-browser debug breakpoint set <script-id> <line> [options]
agent-browser debug breakpoint list|remove <probe-id>
agent-browser debug logpoint set <script-id> <line> --expression <js>...
agent-browser debug logpoint list|remove <probe-id>
agent-browser debug status|stack|resume [selectors]
agent-browser debug eval <expression> [--frame <index>]
agent-browser debug events [--since <sequence>] [--wait <ms>] [--clear]
```

See [Debugging](/debugging) for compiled JavaScript breakpoints and logpoints, lock-independent pause recovery, console, error, dialog, trace, highlight, and DevTools workflows. See [Video Recording](/recording) for `record` workflows and [Profiler](/profiler) for performance profiles.
...
```bash
agent-browser state save <path> [--include-origin <url>]...  # Save auth state to file
...
`open`, `goto`, and `navigate` wait up to 60 seconds for the selected navigation lifecycle event by default. Pass `--timeout <ms>` to override that wait for one navigation. `AGENT_BROWSER_DEFAULT_TIMEOUT` also overrides the navigation default. The effective timeout is forwarded to the command transport with an additional response margin.
...
--restore-initial-save <bool> # One post-launch save after about two seconds
--restore-periodic-save <bool> # Continue saving periodically while open
--restore-close-save <bool> # Save before close, shutdown, or relaunch
--restore-periodic-save-interval-ms <ms> # Minimum periodic interval; 0 disables periodic only
...
- `debug` - WebMCP list/call, console/errors, tracing, profiling, recording, accessibility audits, clipboard, plugins, doctor, dashboard, install, upgrade, chat, diff, batch, confirm/deny
