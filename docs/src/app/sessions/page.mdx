...
State files preserve HTTP-only and partitioned cookie metadata. Cookies are collected for the whole browser session. Repeat `--include-origin` when an authentication origin has localStorage that the active agent-browser session did not visit:
...
State replay restores localStorage and sessionStorage through intercepted blank responses, so saved origins are not contacted before the requested navigation.
...
State files are stored in `~/.agent-browser/sessions/` and automatically loaded before navigation. They contain cookies, localStorage, and sessionStorage. They are not complete Chrome Profiles, which also keep IndexedDB, service workers, cache, extensions, preferences, and other Chrome-owned data. With the default `--restore-save auto` policy, failed restore or failed validation skips saving.

When `--restore` or another restore key is configured, three independent stages control saving. By default, agent-browser saves once after a newly launched page has been quiet for about two seconds, saves periodically while the browser stays open, and saves before explicit `close`, idle timeout, daemon shutdown, or a compatible relaunch. A session ID by itself only isolates the daemon and does not enable persistence; without a restore key, shutdown discards transient browser state and open tabs.

Use `--restore-initial-save false` to disable only the post-launch save, `--restore-periodic-save false` to disable later periodic saves, and `--restore-close-save false` to disable lifecycle saving. Periodic saves run at most once per `--restore-periodic-save-interval-ms` or `AGENT_BROWSER_AUTOSAVE_INTERVAL_MS` (default 30000); `0` disables only periodic saves. Idle sessions remain eligible when periodic saving is enabled, so page-driven token refreshes and background changes are captured. These fields are sent with each command, so changing them works with an already-running daemon.

Cross-origin storage collection uses a disposable CDP target. agent-browser requests a background target when supported and falls back to the compatible foreground form. In a headed browser, that fallback can briefly show the temporary target; the original page is not refreshed. Disabling the initial stage avoids the post-launch switch while keeping close-time saving. `--restore-save never` is different because it disables initial, periodic, and close saving together.

The daemon exits after one hour without commands or dashboard input by default; `--idle-timeout <time>` or `AGENT_BROWSER_IDLE_TIMEOUT_MS` tunes this, and `0` disables it. Headed, Safari/iOS WebDriver, and user-attached browsers are exempt from the default timeout; provider-owned cloud browsers are not.
...
agent-browser state save ./backup.json --include-origin https://sso.example.com
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>AGENT_BROWSER_RESTORE_INITIAL_SAVE</code></td><td>Enable the one-time post-launch save (default: true)</td></tr>
    <tr><td><code>AGENT_BROWSER_RESTORE_PERIODIC_SAVE</code></td><td>Enable periodic saves while open (default: true)</td></tr>
    <tr><td><code>AGENT_BROWSER_RESTORE_CLOSE_SAVE</code></td><td>Enable saves before close, shutdown, or relaunch (default: true)</td></tr>
    <tr><td><code>AGENT_BROWSER_AUTOSAVE_INTERVAL_MS</code></td><td>Minimum ms between periodic saves (default: 30000; <code>0</code> disables periodic only)</td></tr>
