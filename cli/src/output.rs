...
fn format_debug_text(action: Option<&str>, data: &serde_json::Value) -> Option<String> {
    ...
    match action {
        "debug_enable" => Some(format!(
            "{} JavaScript debugger enabled for {} session(s)",
            color::success_indicator(),
            data.get("sessions")
        ...
        "debug_disable" => Some(format!(
            "{} JavaScript debugger disabled for {} session(s)",
            color::success_indicator(),
            data.get("sessions")
                .and_then(|value| value.as_array())
                .map(Vec::len)
        ...
        "debug_source" => data
            ...
            .and_then(|value| value.as_str())
            .map(ToString::to_string),
        "debug_breakpoint_remove" | "debug_logpoint_remove" => Some(format!(
            "{} Removed {}",
            color::success_indicator(),
            data.get("probeId")
                .and_then(|value| value.as_str())
        ...
        _ => serde_json::to_string_pretty(data).ok(),
    }
}
